...
import com.example.mydictionary.Util.RECENT_LIST_STORAGE_KEY
...
class DictionaryViewModel : ViewModel() {
    ...
    fun searchKeyword(text: String?) {
        if (text != null && text.isNotEmpty()) {
            ...
            if (trans == "/") {
                keyword = ""
                result.value =
                    "Translation of \"$text\" is not available, please try again with another keyword"
            } else {
                keyword = text
                result.value = trans
            }
    ...
    fun putStringToSP(newValue: String) {
        val list = kmmStorage.getList(RECENT_LIST_STORAGE_KEY)
        if (list.size >= 20) {
            list.removeAt(19)
        }
        if (list.contains("")) {
        ...
        kmmStorage.setList(RECENT_LIST_STORAGE_KEY, sb.toString())
    }

    fun checkSPExist(context: Context) {
        val sharedPreferences = context.getSharedPreferences(SP_NAME, MODE_PRIVATE)
        if (!sharedPreferences.contains(RECENT_LIST_STORAGE_KEY)) {
            val editor = sharedPreferences.edit()
            editor.putString(RECENT_LIST_STORAGE_KEY, "Hello")
            editor.apply()
        }
    }
}